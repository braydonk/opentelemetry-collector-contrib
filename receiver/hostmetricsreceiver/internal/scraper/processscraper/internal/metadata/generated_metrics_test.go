		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "system.processes.count":
					...
					assert.Equal(t, pmetric.MetricTypeSum, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Sum().DataPoints().Len())
					assert.Equal(t, "Total number of processes in each state on the system.", ms.At(i).Description())
				...
				case "system.processes.created":
					...
					assert.Equal(t, pmetric.MetricTypeSum, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Sum().DataPoints().Len())
					assert.Equal(t, "Total number of created processes on the system.", ms.At(i).Description())
					assert.Equal(t, "{processes}", ms.At(i).Unit())
					assert.Equal(t, true, ms.At(i).Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, ms.At(i).Sum().AggregationTemporality())
					dp := ms.At(i).Sum().DataPoints().At(0)
